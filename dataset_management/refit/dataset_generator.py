...
class DatasetGenerator:
    ...
    __agg_mean (int): The mean value used to normalize the aggregate
    power data.
    __agg_std (int): The standard deviation used to normalize the
    ...
    def __init__(self, appliance_name="microwave"):
        self.__appliance = appliance_name
        self.__directory = "G:/refit/"
        self.__agg_mean = 522
        self.__agg_std = 814
    ...
    def generate_test_house(self):
        """Normalizes the aggregate and appliance data for a specified house to be part of the testing set.
        Writes this data to the testing file."""

        print("Formatting " + self.__appliance + " test data...")

        # Load the test data.
        test_data = self.load_file(
            appliance_data[self.__appliance]["test_house"],
            appliance_data[self.__appliance]["channels"][
                appliance_data[self.__appliance]["houses"].index(
                    appliance_data[self.__appliance]["test_house"]
                )
            ],
        )

        # Normalize the appliance's test data.
        test_data["aggregate"] = (
            test_data["aggregate"] - self.__agg_mean
        ) / self.__agg_std
        test_data[self.__appliance] = (
            test_data[self.__appliance] - appliance_data[self.__appliance]["mean"]
        ) / appliance_data[self.__appliance]["std"]

        # Save the test data.
        test_data.to_csv(
            "./" + self.__appliance + "/" + self.__appliance + "_test_.csv", index=False
        )
    ...
    def generate_validation_house(self):
        """Normalizes the aggregate and appliance data for a specified house to be part of the validation set. Writes
        this data to the validation file."""

        print("Formatting " + self.__appliance + " validation data...")

        # Load the validation data.
        validation_data = self.load_file(
            appliance_data[self.__appliance]["validation_house"],
            appliance_data[self.__appliance]["channels"][
                appliance_data[self.__appliance]["houses"].index(
                    appliance_data[self.__appliance]["validation_house"]
                )
        ...
        validation_data["aggregate"] = (
            validation_data["aggregate"] - self.__agg_mean
        ) / self.__agg_std
        validation_data[self.__appliance] = (
            validation_data[self.__appliance] - appliance_data[self.__appliance]["mean"]
        ) / appliance_data[self.__appliance]["std"]

        # Save validation data.
        validation_data.to_csv(
            "./" + self.__appliance + "/" + self.__appliance + "_validation_.csv",
            index=False,
        )
    ...
    def generate_train_house(self, file_name):
        """Normalizes the aggregate and appliance data for a specified house to be part of the training set. Writes
        ...
        try:
            training_data = self.load_file(
                self.digits_in_file_name(file_name),
                appliance_data[self.__appliance]["channels"][
                    appliance_data[self.__appliance]["houses"].index(
                        self.digits_in_file_name(file_name)
                    )
            ...
            training_data["aggregate"] = (
                training_data["aggregate"] - self.__agg_mean
            ) / self.__agg_std
            training_data[self.__appliance] = (
                training_data[self.__appliance]
                - appliance_data[self.__appliance]["mean"]
            ) / appliance_data[self.__appliance]["std"]
            ...
            training_data.to_csv(
                "./" + self.__appliance + "/" + self.__appliance + "_training_.csv",
                mode="a",
                index=False,
                header=False,
            )
    ...
    def generate(self):
        """Generates normalized training, validation, and testing datasets from the cleaned REFIT dataset."""
        ...
        for _, file_name in enumerate(os.listdir(self.__directory)):
            ...
            if file_name.startswith(
                "House_" + str(appliance_data[self.__appliance]["test_house"])
            ):
                self.generate_test_house()
            # Format the appliance's validation data.
            elif file_name.startswith(
                "House_" + str(appliance_data[self.__appliance]["validation_house"])
            ):
                self.generate_validation_house()
            # Format training data.
            elif file_name.startswith("House_"):
                self.generate_train_house(file_name)

        print(
            "The training dataset contains "
            + str(self.__training_set_length)
            + " rows of data."
        )
